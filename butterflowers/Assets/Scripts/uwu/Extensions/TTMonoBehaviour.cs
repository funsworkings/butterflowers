...
namespace uwu.Extensions
{
	...
	{
		LockQueue LockedCoroutineQueue { get; set; }
		...
		{
			var coroutineObj = new Coroutine<T>();
			coroutineObj.coroutine = base.StartCoroutine(coroutineObj.InternalRoutine(coroutine));
			return coroutineObj;
		}

		/// <summary>
		///     Lockable coroutine. Can either wait for a previous coroutine to finish or a timeout or just bail if previous one
		///     isn't done.
		///     Caution: the default timeout is 10 seconds. Coroutines that timeout just drop so if its essential increase this
		///     timeout.
		...
		public Coroutine<T> StartCoroutine<T>(IEnumerator coroutine, string lockID, float waitTime = 10f)
		{
			...
			var coroutineObj = new Coroutine<T>(lockID, waitTime, LockedCoroutineQueue);
			coroutineObj.coroutine = base.StartCoroutine(coroutineObj.InternalRoutine(coroutine));
			return coroutineObj;
		}
		...
		public class Coroutine<T>
		{
			...
			readonly LockQueue lockedCoroutines; //reference to objects lockdict
			...
			public IEnumerator InternalRoutine(IEnumerator coroutine)
			{
				if (lockable && lockedCoroutines != null) {
					if (lockedCoroutines.Contains(lockID)) {
						...
						if (counter >= waitTime) {
							var error = GetType().Name + ": coroutine " + lockID + " bailing! due to timeout: " + counter;
				...
				while (true) {
					try {
						...
						this.e = e;
						Debug.LogError(GetType().Name + ": caught Coroutine exception! " + e.Message + "\n" + e.StackTrace);
						if (lockable) lockedCoroutines.Remove(lockID, coroutine);
						yield break;
					}
		...
		public class LockQueue
		{
			...
			Dictionary<string, List<IEnumerator>> LockedCoroutines { get; }
			...
			{
				if (!LockedCoroutines.ContainsKey(lockID)) LockedCoroutines.Add(lockID, new List<IEnumerator>());

				if (!LockedCoroutines[lockID].Contains(coroutine)) LockedCoroutines[lockID].Add(coroutine);
			}
			...
			public bool Remove(string lockID, IEnumerator coroutine)
			{
				...
				if (LockedCoroutines.ContainsKey(lockID)) {
					if (LockedCoroutines[lockID].Contains(coroutine)) ret = LockedCoroutines[lockID].Remove(coroutine);

					if (LockedCoroutines[lockID].Count == 0) ret = LockedCoroutines.Remove(lockID);
				}
