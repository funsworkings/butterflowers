...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow : EditorWindow {
        ...
        public static void Open(XNode.NodeGraph graph, string name) {
            if (!graph) return;

            NodeEditorWindow w = GetWindow(typeof(NodeEditorWindow), false, string.IsNullOrEmpty(name)?"xNode":name, true) as NodeEditorWindow;
            w.wantsMouseMove = true;
            w.graph = graph;
        }
